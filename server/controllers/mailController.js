...
const getMails = asyncHandler(async (req, res) => {
  ...
  const allMails = await Mail.find({}).populate(
    "user",
    "firstName lastName email"
  );

  if (!allMails || !userMail) {
    res.json({ mails: [] });
    return;
  }

  const inMails = allMails.filter(
    (mail) => mail.mailTarget === userMail.email && !mail.deletedIn
  );
  const outMails = allMails.filter(
    (mail) => mail.user._id.toString() === userId && !mail.deletedOut
  );

  res.json({ inMails, outMails });
});
